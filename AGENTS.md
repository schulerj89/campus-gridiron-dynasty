...
- National season award statue assets live in `public/assets/award-statues`; keep them fictional generated bronze football trophies with one image per award key, not local placeholder drawings.
...
- After the Crown Bowl final, the Overview dashboard should stop on a championship recap checkpoint; the next advance should open the active offseason stage panels.
...
- Spending Program Investment points must immediately reconcile active recruiting budgets while preserving sunk spent points; training and facilities investment must materially affect preseason development.
- Spending coach points on recruiting must immediately reconcile active recruiting budgets while preserving sunk spent points.
...
- `src/components/AwardsView.tsx` owns awards, honor-team, and playoff bracket presentation.
- `src/components/StatsView.tsx` owns stat leaderboards, stat scope filtering, and completion percentage display rules.
- `src/components/ProgramHistoryView.tsx` owns derived program record book and dynasty history presentation.
